...
describe('sseToPanelEvent (Phase 7.2)', () => {
  ...
  // -------------------------------------------------------------------------
  // Siri-Ray round-3 P1 on PR #1314: the variant-level validator used to
  // accept any `typeof === 'string'` for closed-enum fields (ship.status,
  // panelist_*.role, degraded.reason, failed.cause, parser_warning.kind,
  // run_started.cast[]) and any `typeof === 'number'` for numeric fields
  // (composite, score, dimScore) which let NaN / Infinity through.
  // Downstream component code does `SHIP_BADGE_KEY[status]`, etc., which
  // returns undefined on unknown values and crashes the i18n call.
  // -------------------------------------------------------------------------

  it('drops a critique.ship frame with an unknown status value', () => {
    expect(
      sseToPanelEvent('critique.ship' as CritiqueSseEventName, {
        runId: 'r',
        round: 1,
  ...
  it('drops a critique.ship frame with a non-finite composite', () => {
    expect(
      sseToPanelEvent('critique.ship' as CritiqueSseEventName, {
        runId: 'r',
        round: 1,
  ...
  it('drops a critique.panelist_open frame with an unknown role value', () => {
    expect(
      sseToPanelEvent('critique.panelist_open' as CritiqueSseEventName, {
        runId: 'r',
        round: 1,
        role: 'overlord',
      }),
  ...
  it('drops a critique.panelist_close frame with a non-finite score', () => {
    expect(
  ...
  it('drops a critique.degraded frame with an unknown reason value', () => {
    expect(
      sseToPanelEvent('critique.degraded' as CritiqueSseEventName, {
        runId: 'r',
        reason: 'weird_problem',
        adapter: 'pi-rpc',
      }),
    ).toBeNull();
  });

  it('drops a critique.failed frame with an unknown cause value', () => {
    expect(
      sseToPanelEvent('critique.failed' as CritiqueSseEventName, {
        runId: 'r',
        cause: 'who_knows',
      }),
    ).toBeNull();
  ...
  it('drops a critique.parser_warning frame with an unknown kind value', () => {
    expect(
      sseToPanelEvent('critique.parser_warning' as CritiqueSseEventName, {
        runId: 'r',
        kind: 'something_new',
        position: 0,
      }),
  ...
  it('drops a critique.run_started frame whose cast carries an unknown role', () => {
    expect(
      sseToPanelEvent('critique.run_started' as CritiqueSseEventName, {
        runId: 'r',
        protocolVersion: 1,
        cast: ['critic', 'overlord'],
        maxRounds: 3,
        threshold: 8,
        scale: 10,
      }),
  ...
  it('drops a critique.run_started frame with a non-finite scale', () => {
    expect(
      sseToPanelEvent('critique.run_started' as CritiqueSseEventName, {
        runId: 'r',
        protocolVersion: 1,
  ...
  it('accepts every well-formed enum value across the closed sets', () => {
    // Coverage that the strict guard does not over-reject. One frame per
    // closed-set field with each legal value.
    for (const status of ['shipped', 'below_threshold', 'timed_out', 'interrupted'] as const) {
      expect(
        sseToPanelEvent('critique.ship' as CritiqueSseEventName, {
          runId: 'r', round: 1, composite: 8.6, status,
          artifactRef: { projectId: 'p', artifactId: 'a' }, summary: 'ok',
        }),
      ).not.toBeNull();
    }
    for (const reason of [
      'malformed_block', 'oversize_block', 'adapter_unsupported',
      'protocol_version_mismatch', 'missing_artifact',
    ] as const) {
      expect(
        sseToPanelEvent('critique.degraded' as CritiqueSseEventName, {
          runId: 'r', reason, adapter: 'pi-rpc',
        }),
      ).not.toBeNull();
    }
    for (const cause of [
      'cli_exit_nonzero', 'per_round_timeout', 'total_timeout', 'orchestrator_internal',
    ] as const) {
      expect(
        sseToPanelEvent('critique.failed' as CritiqueSseEventName, {
          runId: 'r', cause,
    ...
    for (const kind of [
      'weak_debate', 'unknown_role', 'score_clamped', 'composite_mismatch', 'duplicate_ship',
    ] as const) {
      expect(
        sseToPanelEvent('critique.parser_warning' as CritiqueSseEventName, {
          runId: 'r', kind, position: 12,
        }),
      ).not.toBeNull();
    }
    for (const role of ['designer', 'critic', 'brand', 'a11y', 'copy'] as const) {
      expect(
        sseToPanelEvent('critique.panelist_open' as CritiqueSseEventName, {
          runId: 'r', round: 1, role,
        }),
      ).not.toBeNull();
    }
  });
